...
import dev.openrune.cache.worldmap.worldmap.WORLD_MAP_GROUND_ARCHIVE
import dev.openrune.cache.worldmap.worldmap.WorldMapArea
import dev.openrune.cache.worldmap.worldmap.WorldMapFormat
import dev.openrune.cache.worldmap.worldmap.ground.NeighbourPosition.*
import dev.openrune.cache.worldmap.worldmap.providers.*
import dev.openrune.cache.worldmap.worldmap.rasterizer.WorldMapRenderer.drawOverlaysAndElements
import dev.openrune.cache.worldmap.worldmap.rasterizer.WorldMapRenderer.generateCompositeTexture
import dev.openrune.cache.worldmap.worldmap.toImage
    ...
    companion object {
        ...
        fun generateMapImage(
            ...
            val images = if (generateUnderlays) {
                ...
                parseUnderlayImages(providers.cacheProvider, groundAreas, areaData, revision)
            }
            val mapSceneSprites = MapSceneSprites.build(providers.graphicsDefaultsProvider, providers.spriteProvider)
        ...
        private fun parseUnderlayImages(
            ...
            val areaId = areaData.details.id
            val boundaries = areaData.boundaries
            ...
            for (mapsquare in areaData.data.mapsquares) {
                ...
                val groundGroup = if (legacy) {
                    mapsquare.data.groupId
                } else {
                    WorldMapFormat.regionGroupKey(
                        mapsquare.data.mapsquareDestinationX,
                        mapsquare.data.mapsquareDestinationY
                    )
                }
                val groundFile = if (legacy) mapsquare.data.fileId else areaId
                if (!cacheProvider.exists(WORLD_MAP_GROUND_ARCHIVE, groundGroup, groundFile)) continue
                val buffer = cacheProvider.read(WORLD_MAP_GROUND_ARCHIVE, groundGroup, groundFile)
            ...
            for (x in 0 until totalWidth) {
                for (y in 0 until totalHeight) {
                    ...
                    val groundGroup = if (legacy) {
                        ...
                        WorldMapFormat.regionGroupKey(mapsquareDestinationX, mapsquareDestinationY)
                    }
                    val groundFile = if (legacy) 0 else areaId
                    if (!cacheProvider.exists(WORLD_MAP_GROUND_ARCHIVE, groundGroup, groundFile)) continue
                    val buffer = cacheProvider.read(WORLD_MAP_GROUND_ARCHIVE, groundGroup, groundFile)
