...
/**
 * Decodes every map scene icon (MSI) for every "runescape" build the config
 ...
 * `./gradlew :filestore:rs2-fs:test --tests dev.openrune.Rs2MapSceneIconScanTest -Dmsiscan=true --rerun-tasks`
 */
@EnabledIfSystemProperty(named = "msiscan", matches = "true")
class Rs2MapSceneIconScanTest {

    @Test
    fun `scan every runescape build for map scene icon decode failures`() {
        ...
        val results = caches.mapIndexed { index, entry ->
            ...
            val status = try {
                Rs2Cache.loadRemote(scope = entry.scope, id = entry.id).use { cache ->
                    val ids = cache.mapSceneIconIds()
                    for (id in ids) cache.readMapSceneIcon(id)
                    Status.Ok(ids.size)
                }
            ...
            val label = when (status) {
                is Status.Ok -> "OK (${status.count} icons)"
        ...
        println("\n$ok/${results.size} builds decoded map scene icons successfully, $skipped skipped (group not present), ${failed.size} failed.")
    ...
    private fun writeReport(results: List<ScanResult>) {
        val file = File("build/msi-scan", "report.md").apply { parentFile.mkdirs() }
        val ok = results.count { it.status is Status.Ok }
        val skipped = results.count { it.status is Status.Skipped }
        val failed = results.filter { it.status is Status.Failed }

        val lines = buildList {
            add("# RS2/RS3 map scene icon decode scan")
            add("")
