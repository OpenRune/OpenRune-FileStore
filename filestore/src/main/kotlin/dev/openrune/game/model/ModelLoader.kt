package dev.openrune.game.model
...
class ModelLoader(val bytes : ByteArray,val modelId: Int) {

    companion object {
        fun provide(modelId : Int) : ModelDefinition {
            return ModelLoader(CacheManager.cache.data(MODELS, modelId)!!,modelId).get()
        }
    }


    fun get(): ModelDefinition {

        val modelDefinition = ModelDefinition()
        modelDefinition.id = modelId
        if (bytes[bytes.size - 1].toInt() == -3 && bytes[bytes.size - 2].toInt() == -1) {
            decodeType3(modelDefinition, bytes)
        } else if (bytes[bytes.size - 1].toInt() == -2 && bytes[bytes.size - 2].toInt() == -1) {
            decodeType2(modelDefinition, bytes)
        } else if (bytes[bytes.size - 1].toInt() == -1 && bytes[bytes.size - 2].toInt() == -1) {
            load1(modelDefinition, bytes)
        } else {
            loadOldFormat(modelDefinition, bytes)
        }
