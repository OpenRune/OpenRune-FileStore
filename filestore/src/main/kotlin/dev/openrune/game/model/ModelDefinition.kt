...
import dev.openrune.game.model.data.VertexNormal
import dev.openrune.game.model.data.CircularAngle
import dev.openrune.game.model.data.FaceNormal
import java.util.*
...
//        faceTextureUCoordinates = null
    ...
    fun recolor(var1: Int, var2: Int) {
        for (var3 in 0 until faceCount) {
            if (faceColors!![var3].toInt() == var1) {
                faceColors!![var3] = var2.toShort()
            }
        }
    }

    fun retexture(var1: Int, var2: Int) {
        if (faceTextures != null) {
            for (var3 in 0 until faceCount) {
                if (faceTextures!![var3].toInt() == var1) {
                    faceTextures!![var3] = var2.toShort()
                }
            }
        }
    }
