...
object GameValHandler {
    ...
    fun readGameVal(type: GameValGroupTypes, cache: Cache, cacheRevision : Int = -1): List<GameValElement> {
        ...
        if (type.revision != -1) {
            val rev = if (cacheRevision == -1) readCacheRevision(cache,"${type.name} is Unlivable in this revision") else cacheRevision
            if (rev < type.revision) {
                error("${type.name} is Unlivable in this revision")
            }
        }
