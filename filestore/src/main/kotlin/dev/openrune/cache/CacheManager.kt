...
fun <T : Definition> MutableMap<Int, T>.withOffset(offset: Int): MutableMap<Int, T> {
    ...
    // Single pass into a pre-sized map; `mapKeys { }.toMutableMap()` built two full copies.
...
object CacheManager {
    ...
    /**
     * Adopts [source] outright when nothing has been loaded yet, otherwise merges into the existing
     * map. The first init is the overwhelmingly common case, and copying there doubled the resident
     * size of every definition table for no benefit.
     */
    ...
    /**
     * Same contract as [getOrDefault] but the fallback is only built on a miss. The eager form
     * allocated a throwaway definition on every single lookup, hit or not.
     */
    ...
    // Bulk getters. These hand back a read-only view rather than a defensive copy - the tables are
    // immutable after `init` and copying 60k+ entries per call showed up in every dump/export loop.
