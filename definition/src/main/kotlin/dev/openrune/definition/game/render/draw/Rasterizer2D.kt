@file:Suppress(
    "PrivatePropertyName", "FunctionName", "PropertyName", "unused",
    "MemberVisibilityCanBePrivate", "DuplicatedCode", "NAME_SHADOWING",
)
...
/**
 * Software raster target: owns a pixel buffer, a clip rectangle, and the 2D drawing operations that
 * write into them. [Rasterizer3D] extends this, so anything holding a 3D rasterizer can also use the
 * 2D operations directly.
 *
 * The clip rectangle is exposed under two sets of names: [drawRegionX] and friends are the long form,
 * [minX] and friends are the short aliases the drawing operations are written against. Both refer to
 * the same state.
 *
...
open class Rasterizer2D {
    ...
    var graphicsPixels: IntArray = emptyIntArray
    ...
    var pixels: IntArray
        get() = graphicsPixels
        set(value) { graphicsPixels = value }
    var width: Int
        get() = graphicsPixelsWidth
        set(value) { graphicsPixelsWidth = value }
    var height: Int
        get() = graphicsPixelsHeight
        set(value) { graphicsPixelsHeight = value }
    var minX: Int
        get() = drawRegionX
        set(value) { drawRegionX = value }
    var minY: Int
        get() = drawingAreaTop
        set(value) { drawingAreaTop = value }
    var maxX: Int
        get() = drawingAreaRight
        set(value) { drawingAreaRight = value }
    var maxY: Int
        get() = drawingAreaBottom
        set(value) { drawingAreaBottom = value }

    constructor()

    constructor(width: Int, height: Int) {
        replace(IntArray(width * height), width, height)
    }
